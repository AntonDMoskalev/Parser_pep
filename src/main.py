...
from utils import difference_status, find_tag, get_response


def whats_new(session):
    """
    Collects information about Python innovations.
    """
...
def latest_versions(session):
    """
    Collects information about Python versions.
    """
...
def download(session):
    """
    Downloads an archive with Python documentation.
    """
...
def pep(session):
    """
    Collects data about all PEP documents.
    """
    ...
    for value in tqdm(values):
        ...
        soup = BeautifulSoup(response_card.text, features='lxml')
        table_card = find_tag(soup, 'dl',
                              attrs={'class': 'rfc2822 field-list simple'})
        status_tag = find_tag(table_card, 'dt', string='Status')
